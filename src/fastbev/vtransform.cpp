...
static void compute_volum_kernel(int num_valid, const float* camera_feature, const float* valid_index, const int64_t* valid_y, const int64_t* valid_x, int num_camera, int feat_height, int feat_width, float* output_feature, int32_t tid) {
...
class VtransformImplement : public VTransform {
 public:
  virtual ~VtransformImplement() {
    if (output_feature_) fastbev::Utils::freeTensorMem(output_feature_);//checkRuntime(cudaFree(output_feature_));
    if (input_feature_) fastbev::Utils::freeTensorMem(input_feature_); //checkRuntime(cudaFreeHost(input_feature_));
    if (valid_index_device_) fastbev::Utils::freeTensorMem(valid_index_device_); //checkRuntime(cudaFree(valid_index_device_));
    if (valid_x_device_) fastbev::Utils::freeTensorMem(valid_x_device_);//checkRuntime(cudaFree(valid_x_device_));
    if (valid_y_device_) fastbev::Utils::freeTensorMem(valid_y_device_);//checkRuntime(cudaFree(valid_y_device_));
  }
  ...
  bool init(const std::vector<int>& feature_shape, GeometryParameter param) {
    ...
    input_feature_ = fastbev::Utils::allocTensorMem<float>(volumn_ * sizeof(float)); //checkRuntime(cudaMallocHost(&input_feature_, volumn_ * sizeof(half)));
    ...
    output_feature_ = fastbev::Utils::allocTensorMem<float>(volumn_output_ * sizeof(float));
    ...
    valid_index_device_ = fastbev::Utils::allocTensorMem<float>(bytes_of_valid_index_);
    ...
    valid_y_device_ = fastbev::Utils::allocTensorMem<int64_t>(bytes_of_x_or_y_index_);
    valid_x_device_ = fastbev::Utils::allocTensorMem<int64_t>(bytes_of_x_or_y_index_);
  ...
  virtual void update(const float* valid_c_idx, const int64_t* valid_x, const int64_t* valid_y, void* stream = nullptr) override {
    ...
    std::memcpy(valid_index_device_, valid_c_idx, bytes_of_valid_index_);
    std::memcpy(valid_y_device_, valid_y, bytes_of_x_or_y_index_);
    std::memcpy(valid_x_device_, valid_x, bytes_of_x_or_y_index_);
  ...
  virtual nvtype::half* forward(const nvtype::half* camera_feature, void* stream = nullptr) override {
    //cudaStream_t _stream = static_cast<cudaStream_t>(stream);
    std::memset(output_feature_, 0.0, volumn_output_ * sizeof(float));
    ...
    for(int i=0; i<param_.valid_points; i++){
      compute_volum_kernel(param_.valid_points, reinterpret_cast<const float*>(camera_feature), valid_index_device_, valid_y_device_, valid_x_device_, param_.num_camera, param_.feat_height, param_.feat_width, output_feature_, i);
    }
 ...
 private:
  ...
  float* output_feature_ = nullptr;
  float* input_feature_ = nullptr;
